...
/**
 * StatCard — lightweight stat display card.
 * ParticleBurst removed (issue #7): replaced with CSS shine + scale hover.
 */
const StatCard = ({
  title,
  value,
    ...
    >
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-2 sm:mb-3">
          {Icon && (
            <motion.div
              className="w-10 h-10 sm:w-12 sm:h-12 bg-white/20 rounded-xl flex items-center justify-center backdrop-blur-sm"
          ...
          {trend && (
            <motion.span
              className="text-xs font-semibold px-2 py-1 rounded-full bg-white/20"
        ...
        >
          {animateValue && !isNaN(numericValue) ? (
            <AnimatedCounter value={numericValue} prefix={prefix} suffix={suffix} decimals={2} />
          ) : (
            value
          )}
        </motion.p>
