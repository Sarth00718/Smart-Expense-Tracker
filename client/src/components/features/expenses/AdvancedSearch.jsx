...
const AdvancedSearch = ({ onSearch, onClose }) => {
  ...
  const handleSaveFilter = async () => {
    ...
    try {
      await api.post('/filters', { name: filterName, filters });
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl p-6 max-w-4xl mx-auto border border-gray-200 dark:border-slate-700">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center">
          <Filter className="w-6 h-6 text-blue-600 dark:text-blue-400 mr-2" />
          <h2 className="text-xl font-semibold text-gray-800 dark:text-slate-100">Advanced Search</h2>
        </div>
        {onClose && (
      ...
      {/* Quick Filters */}
      <div className="mb-6">
        <h3 className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-3">Quick Filters</h3>
        <div className="flex flex-wrap gap-2">
          {quickFilters.map(qf => (
            <button
              key={qf.value}
              onClick={() => handleQuickFilter(qf.value)}
              className="px-4 py-2 text-sm bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 text-gray-700 dark:text-slate-300 rounded-lg transition-colors font-medium"
            >
              {qf.label}
            </button>
          ))}
      ...
      {/* Categories */}
      <div className="mb-6">
        <label className="flex items-center text-sm font-medium text-gray-700 dark:text-slate-300 mb-3">
        ...
        <div className="flex flex-wrap gap-2">
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 text-sm rounded-lg transition-all font-medium ${filters.categories.includes(cat)
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button
          ...
          className="px-4 py-3 border-2 border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors"
          title="Save Filter"
        >
          <Save className="w-5 h-5 text-gray-600 dark:text-slate-400" />
        </button>
        <button
          onClick={handleClearFilters}
          className="px-6 py-3 border-2 border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors font-medium text-gray-700 dark:text-slate-300"
        >
          Clear
        </button>
      </div>
      ...
      {savedFilters.length > 0 && (
        <div className="mt-6 pt-6 border-t border-gray-200 dark:border-slate-700">
          <h3 className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-3">Saved Filters</h3>
          <div className="flex flex-wrap gap-2">
            {savedFilters.map(sf => (
              <button
                key={sf._id}
                onClick={() => handleLoadFilter(sf)}
                className="px-4 py-2 text-sm bg-purple-100 dark:bg-purple-900/30 hover:bg-purple-200 dark:hover:bg-purple-900/50 text-purple-700 dark:text-purple-300 rounded-lg transition-colors font-medium"
              >
                {sf.name}
              </button>
            ))}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 max-w-sm w-full mx-4 shadow-2xl border border-gray-200 dark:border-slate-700">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="px-4 py-2.5 border-2 border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors font-medium text-gray-700 dark:text-slate-300"
              >
                Cancel
              </button>
            </div>
