...
const MobileNav = () => {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-40 safe-area-bottom shadow-2xl">
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <span className={`text-[10px] sm:text-xs font-medium ${isActive ? 'font-bold' : ''} truncate max-w-full px-1`}>
                  {item.label}
                </span>
              </>
            )}
