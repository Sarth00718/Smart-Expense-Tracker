...
export const firebaseSync = asyncHandler(async (req, res) => {
  ...
  res.status(statusCode).json({ success: true, message: result.message, data: { token: result.token, user: result.user } });
});
...
export const linkFirebase = asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, message: result.message, data: { user: result.user } });
});
