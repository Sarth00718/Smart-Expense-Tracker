...
export const getAllGoals = async (req, res) => {
  try {
    ...
    const result = goals.map((goal) => {
      ...
      let daysLeft = null, neededPerDay = 0;
      if (goal.deadline) {
        daysLeft = Math.max(0, Math.ceil((new Date(goal.deadline) - new Date()) / (1000 * 60 * 60 * 24)));
...
export const createGoal = async (req, res) => {
  try {
    ...
    if (targetAmount <= 0) return res.status(400).json({ error: 'Target amount must be positive' });
...
export const getStats = async (req, res) => {
  try {
    ...
    res.json({ total_goals: 0, completed_goals: 0, total_target: 0, total_current: 0, overall_percentage: 0, remaining_amount: 0 });
  }
};
