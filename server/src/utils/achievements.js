...
const BADGE_INFO = {
  first_expense:      { title: 'First Step',            icon: '🎯', description: 'Added your first expense' },
  expense_master:     { title: 'Expense Master',         icon: '📊', description: 'Added 50+ expenses' },
  category_explorer:  { title: 'Category Explorer',      icon: '🗂️', description: 'Used 5+ categories' },
  goal_achiever:      { title: 'Goal Achiever',          icon: '🎖️', description: 'Completed a savings goal' },
  budget_master:      { title: 'Budget Master',          icon: '💰', description: 'Stayed under all budgets' },
  super_saver:        { title: 'Super Saver',            icon: '🏆', description: 'Saved 20%+ of income' },
  tracking_streak_7:  { title: '7-Day Tracking Streak',  icon: '🔥', description: 'Tracked expenses for 7 days' },
  receipt_pro:        { title: 'Receipt Pro',            icon: '📸', description: 'Scanned 5+ receipts' },
};

export function getBadgeInfo(badgeName) {
  return BADGE_INFO[badgeName] || { title: badgeName, icon: '🏅', description: 'Achievement earned' };
}
...
export async function checkAndAwardAchievements(userId) {
  ...
  try {
    ...
    if (budgets.length >= 3 && !existingBadges.includes('budget_master')) {
      ...
      for (const budget of budgets) {
        const spent = await Expense.aggregate([
          { $match: { userId, category: budget.category, date: { $gte: startOfMonth } } },
          { $group: { _id: null, total: { $sum: '$amount' } } },
        ]);
        if ((spent[0]?.total || 0) > budget.monthlyBudget) {
    ...
    if (!existingBadges.includes('tracking_streak_7')) {
      const recentExpenses = await Expense.find({ userId }).sort({ date: -1 }).limit(30);
      const uniqueDates = new Set(recentExpenses.map((e) => e.date.toISOString().split('T')[0]));
...
async function awardBadge(userId, badgeName, badgeType) {
  try {
    await new Achievement({ userId, badgeName, badgeType }).save();
  } catch (error) {
    if (error.code !== 11000) {
